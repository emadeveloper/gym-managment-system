WITH templates(template_name, objective, level, days_per_week, estimated_duration_weeks, description) AS (
    VALUES
        ('Fuerza full body inicial', 'Fuerza', 'Principiante', 3, 6, 'Plantilla base para construir técnica y fuerza general.'),
        ('Hipertrofia torso pierna', 'Hipertrofia', 'Intermedio', 4, 8, 'Bloque de volumen con distribución torso pierna.')
),
upserted_templates AS (
    ...
    SELECT
        (
            substr(md5(template_name), 1, 8) || '-' ||
            substr(md5(template_name), 9, 4) || '-' ||
            '4' || substr(md5(template_name), 14, 3) || '-' ||
            'a' || substr(md5(template_name), 18, 3) || '-' ||
            substr(md5(template_name), 21, 12)
        )::uuid,
        template_name,
        objective,
        level,
